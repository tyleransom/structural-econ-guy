using JuMP, Ipopt, Optim, LineSearches, LinearAlgebra, SparseArrays, Distributions, DataFrames, CSV, HTTP, MathOptInterface

#-------------------------------------------------------------------------------
# Read in data from PS8
#-------------------------------------------------------------------------------
...
function jump_mle(θ₀, X, y)
    ...
    hessian_sparsity = MathOptInterface.hessian_lagrangian_structure(d)
    V = zeros(length(hessian_sparsity))
    MathOptInterface.eval_hessian_lagrangian(d, V, values, 1.0, Float64[])
...
#-------------------------------------------------------------------------------
# Constrained optimization using JuMP
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Passing an analytical gradient to Optim
#-------------------------------------------------------------------------------
@views @inline function asclogit(bstart::Vector,Y::Array,X::Array,Z::Array,J::Int64,baseAlt::Int64=J,W::Array=ones(length(Y)))
    ...
    rs = optimize(td, bstart, LBFGS(; linesearch = LineSearches.BackTracking()), Optim.Options(iterations=100_000,g_tol=1e-8,f_tol=1e-8,x_tol=1e-8,show_trace=true, show_every=100))
    β  = Optim.minimizer(rs)
    ℓ  = Optim.minimum(rs)*(-1)
    H  = Optim.hessian!(td, β)
    g  = Optim.gradient!(td, β)
    se = sqrt.(diag(inv(H)))
    println("Converged: ", Optim.converged(rs))
    println("Iterations: ", Optim.iterations(rs))
